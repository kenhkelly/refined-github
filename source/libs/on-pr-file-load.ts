...
const getDeduplicatedHandler = mem((callback: EventListener): delegate.DelegateEventHandler => {
	return (event: delegate.DelegateEvent) => event.delegateTarget.addEventListener('load', callback);
});

export default function onPrFileLoad(callback: EventListener): delegate.DelegateSubscription {
